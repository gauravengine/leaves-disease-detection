...
def activate(request,uidb64,token):
    ...
    if myuser is not None and generate_token.check_token(myuser,token):
        ...
        return HttpResponse("Activation Failed Your Link might be broken")
...
def scan(request):
    ...
    if not request.user.is_authenticated:
        ...
        messages.error(request, "Please Login First to Scan Images")
        return redirect('home')
    ...
    if request.method == 'POST':
        ...
        path = Path.joinpath( settings.MEDIA_ROOT ,'my_image.jpg')
        print(path)
        ...
        del preprocessed_data["f1"]
        p_vector = list(preprocessed_data.values())

        ans=predictor.get_model_and_result(request.POST['plant'],request.POST['model'],p_vector)
        print("+============",request.POST['model'],"==========",request.POST['plant'])
        my_dict={
            'disease':ans,
            'plant_name':request.POST['plant'],
            'model_used':request.POST['model'],
        }
        return render(request,"authentication/result.html", my_dict)
...
def preview(request):
    if not request.user.is_authenticated:
        print("not authenticated")
        messages.error(request, "Please Login First to view Model Statistics")
        return redirect('home')
...
def done(request):
    ...
    messages.error(request, "Due to size limit of free heroku of 500MB we couldn't deploy this feature")
    return redirect('home')
